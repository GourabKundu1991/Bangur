...
const ViewTransactionScreen = ({ navigation, route }) => {
    ...
    const getAllData = (filterValue, startDate, endDate) => {
        AsyncStorage.getItem('userToken').then(val => {
            if (val != null) {
                ...
                formdata.append("created_from", (startDate == "" ? "" : moment(startDate).format("DD-MM-YYYY")));
                formdata.append("created_to", (endDate == "" ? "" : moment(endDate).format("DD-MM-YYYY")));
    ...
    const loadMore = () => {
        ...
        AsyncStorage.getItem('userToken').then(val => {
            if (val != null) {
                ...
                formdata.append("search_value", searchTerms);
                formdata.append("sale_date", "");
                formdata.append("created_from", (fromDate == "" ? "" : moment(fromDate).format("DD-MM-YYYY")));
                formdata.append("created_to", (toDate == "" ? "" : moment(toDate).format("DD-MM-YYYY")));
                                ...
                                <HStack style={MainStyle.inputbox} padding={1} justifyContent="space-between" alignItems="center">
                                    <Pressable width={'38%'} borderRadius={6} backgroundColor={baseLightColor} onPress={() => showDatePicker("from")}>
                                        <HStack height={37} style={{ paddingHorizontal: 6 }} justifyContent="space-between" alignItems="center">
                                            <Text color={fromDate != '' ? '#111111' : '#999999'} fontSize="xs"> {fromDate != '' ? moment(fromDate).format('DD-MM-YYYY') : "From Date"}</Text>
                                    ...
                                    <Pressable width={'38%'} borderRadius={6} backgroundColor={baseLightColor} onPress={() => showDatePicker("to")}>
                                        <HStack height={37} style={{ paddingHorizontal: 6 }} justifyContent="space-between" alignItems="center">
                                            <Text color={toDate != '' ? '#111111' : '#999999'} fontSize="xs"> {toDate != '' ? moment(toDate).format('DD-MM-YYYY') : "To Date"}</Text>
                                            <Icon name="calendar-outline" size={18} color={darkColor} />
                                        </HStack>
                                    </Pressable>
                                ...
                                <Stack key={index} style={{ marginTop: -17, paddingTop: 10 }} borderColor={item.approveStatus == "Pending" ? "#FBC711" : item.approveStatus == "Rejected" ? "#F21515" : "#447A50"} borderLeftWidth={5} backgroundColor={item.approveStatus == "Pending" ? "#FEFFC7" : item.approveStatus == "Rejected" ? "#FFF2F3" : "#F2FFF5"} borderRadius={8} overflow="hidden">
                                    <VStack padding={4}>
                                        <HStack space={6} marginBottom={3} alignItems="center" justifyContent="space-between">
                                            ...
                                            <Text color={darkGrey} fontSize="xs" fontFamily={fontSemiBold} style={{ backgroundColor: lightColor, padding: 3, borderColor: greyColor, borderWidth: 1, borderRadius: 5, overflow: 'hidden' }}> {item.category}</Text>
                                        </HStack>
                                        <HStack alignItems="center" justifyContent="space-between">
                                        ...
                                        <HStack alignItems="center" justifyContent="space-between">
                                            ...
                                            <Text width={'58%'} color={darkColor} fontSize="xs" fontFamily={fontSemiBold} textAlign="right">{moment(item.Sale_Date).format("DD-MM-YYYY")}</Text>
                                        </HStack>
                                        <HStack alignItems="center" justifyContent="space-between">
                                            <Text width={'40%'} color={darkGrey} fontSize="xs" fontFamily={fontRegular}>{t("Uploaded Date")}</Text>
                                            <Text width={'58%'} color={darkColor} fontSize="xs" fontFamily={fontSemiBold} textAlign="right">{moment(item.Sale_Registered_On).format("DD-MM-YYYY")}</Text>
                                        </HStack>
                                        <HStack alignItems="center" justifyContent="space-between">
