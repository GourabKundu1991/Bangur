...
const ViewOrdersScreen = ({ navigation, route }) => {
    ...
    const { rewardId, schemeId } = route.params;
    ...
    return (
        <NativeBaseProvider>
            ...
            <VStack flex={1} backgroundColor={"#000000"}>
                <HeaderComponents component={t("My Orders")} navigation={navigation} />
                <ScrollView backgroundColor={lightColor} borderTopLeftRadius={12} borderTopRightRadius={12} width={'94%'} alignSelf="center">
                    <VStack marginTop={5} style={{ width: '200%', position: 'relative', left: 8 }}>
                        <HStack alignItems="center">
                            <Button size="xs" variant="link" style={[MainStyle.tabbtn, { backgroundColor: filterStatus == "" ? baseColor : lightGrey }]} _text={{ color: filterStatus == "" ? "#ffffff" : darkColor, fontFamily: fontSemiBold, fontSize: 10 }} onPress={() => {
                                setId(rewardId);
                                setFilterStatus("");
                            }}>{t("Redemption")}</Button>
                            <Button size="xs" variant="link" style={[MainStyle.tabbtn, { backgroundColor: filterStatus == "1" ? baseColor : lightGrey }]} _text={{ color: filterStatus == "1" ? "#ffffff" : darkColor, fontFamily: fontSemiBold, fontSize: 10 }} onPress={() => {
                                setId(schemeId);
                                setFilterStatus("1");
                            }}>{t("Scheme")}</Button>
                        </HStack>
